...
class TurnEndForm extends Component {
    ...
    getSummary = async () => {
        ...
        try {
            const {data: {turnSummary: summary}} = await client.query({query: GET_TURN_SUMMARY, fetchPolicy: 'network-only'});
    ...
    handleSubmit = e => {
        const {form, turnActive: {id}, client} = this.props;
    ...
    render() {
        ...
        return(
            <React.Fragment>
                ...
                {summary && <Drawer title="CORTE DE CAJA" width="50%" visible={showSummary} onClose={this.toggleSummary}>
                    <Row style={{margin: 10, padding: 20}} type="flex" justify="space-around" align="middle" gutter={{ xs: 8, sm: 16, md: 24}}>
                        <Column span={6}>
                    ...
                    <Row>
                        <CollapseContainer bordered={false} expandIcon={({ isActive }) => <Icon type="caret-right" rotate={isActive ? 90 : 0} />}>
